...
6. **TARGET SCENE LENGTH: 220–450 characters (~40–80 words).** Each scene should narrate for roughly 10–15 seconds at normal pace.
7. Each scene contains 2 to 5 COMPLETE sentences. Merge them aggressively to hit the 220-character minimum.
8. **EXCEPTION for short closing lines.** If the FINAL piece of script is a single short sentence (< 220 chars) that has no remaining text after it to merge with, append it onto the previous scene instead of making it its own scene.
9. Section headings (e.g. "Part one. The configuration.") should be merged INTO the next scene's text, not stand alone — they're too short to be standalone scenes.

For EACH scene, return a JSON object with:
- "text": the exact verbatim slice of the script (no edits, no punctuation changes). MUST be ≥ 220 characters.
- "visual_prompt": a 40–80-word English prompt for the video generator that LITERALLY illustrates the content of this scene's text, viewed through a cosmic / astronomical lens.
  ...
  • Describe MOTION too — Veo generates 8-second clips so include subtle camera motion (slow zoom, drift, parallax). Example: "slow pan across surface of Mars at dawn, rust-colored dunes stretching to horizon, dust devils on the horizon".
- "duration_hint_sec": approximate length in seconds (number, 10–15).

Return a STRICTLY valid JSON array — no markdown, no explanations.

For a ~1500-word script expect ~25–40 scenes. For a ~700-word script expect ~12–20 scenes. If any "text" field is shorter than 220 chars, the run will fail — RECHECK before returning.`,
